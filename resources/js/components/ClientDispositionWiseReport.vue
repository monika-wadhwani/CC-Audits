	...
	<div class="kt-portlet kt-portlet--mobile">
		...
		<div class="kt-portlet kt-portlet--mobile">
			<div class="kt-portlet__head">
				<div class="kt-portlet__head-label">
					<h3 class="kt-portlet__head-title">
						Report :- <span class="text-danger">Score || Fatal Count</span>						
					</h3>
				</div>
			</div>
			<div class="kt-portlet__body">
				<table class="table table-bordered table-hover" v-if="has_data">
						<thead>
							...
							<tr>
								...
								<td v-for="(des,k1) in audit_count"> {{ des }} || {{ na_count[k1] }}  </td>
								<td>{{ audit_count_total }}</td>
							</tr>
							<tr>
								<th scope="row">Audit Score</th>
								<td v-for="(des1,k2) in audit_score"> {{ des1 }} %  </td>
								<td>{{ audit_score_total }} %</td>
							</tr>
						</thead>
						<tbody v-for="(item,kk) in report_data">
							...
							<tr v-for="(items,kkk) in item.data">								
								...
								<td v-for="(desp,kkk) in despositions">{{items.data[desp].score}} % || {{items.data[desp].fatal_count}}</td>
	    ...
	    methods:{
	    	...
	    	submitFilter: function(e){
	    		...
	    		var params = {
			      ...
			      // console.log(params);
			     axios.post('/report/get_client_disposition_wise_report_data',params).then(function (response){
			     	...
			     	self.na_count = tempt_response.na_counts;		
			     	self.audit_score_total=tempt_response.audit_score_total;
			     	self.audit_count_total=tempt_response.audit_count_total;	     	
			     	self.has_data=true;
                	KTApp.unblockPage();
			      }).catch(function (error) {
			          console.log(error);

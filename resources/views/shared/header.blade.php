...
<!--begin: Notifications -->
<div class="kt-header__topbar-item dropdown" style="display: none;">
...
&nbsp;
	...
	<?php //logged_in_user_total_notifications ?> <span id="noti" style="color:white;"> </span> new</span>
	@else 
...
<div class="kt-notification kt-margin-t-10 kt-margin-b-10 kt-scroll" data-scroll="true" data-height="300" data-mobile-height="200" id="noti_html">
@if(Auth::user()->hasRole('client') ||
                Auth::user()->hasRole('partner-quality-head'))
...
<!--begin: Head -->
<div class="kt-user-card kt-user-card--skin-dark kt-notification-item-padding-x" style="background-image: linear-gradient(to right, #fa709a 0%, #fee140 100%);">
...
$( document ).ready(function() {
	...
	var logged_in_user_notifications=0;
		$.ajax({
			type: "GET",
			url: base_url + "/logged_in_user_total_notifications",
			success: function(response){
				logged_in_user_notifications=response.count;
				if(logged_in_user_notifications != 0) {
					$("#noti").html(logged_in_user_notifications);
					$("#noti_html").html(response.html);
				}
			}
			
		});
});
function redirect_me_noti(noti_id, url) {
	var base_url = window.location.origin;
	$.ajax({
			type: "POST",

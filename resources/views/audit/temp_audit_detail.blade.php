...
@section('main')
										...
										{!! Form::open(
						                  ...
						                  <input type="hidden" name="audit_id" value="{{Crypt::encrypt($audit_data->id)}}">
											<div class="form-group row form-roww">
												...
												<div class="col-lg-4">
													<label>QA Name:</label>
													<input type="text" class="form-control" readonly="readonly" value="{{$raw_data->getQA->name}}">
												</div>
											
												<div class="col-lg-4">
													<label>QA TL Name:</label>
													<input type="text" class="form-control" readonly="readonly" value="{{$audit_data->qa_qtl_detail->get_qtl_name->name}}">
												</div>

												<div class="col-lg-4">
													<label>Campaign Name:</label>
													<input type="text" name = "campaign_name" class="form-control" value="{{$raw_data->campaign_name}}">
												</div>
												<div class="col-lg-4">
													<label>Audit Type:</label>
													<input type="text" class="form-control" name = "audit_type" value="{{$audit_data->audit_type}}">
												</div>
												<div class="col-lg-4">
												...
												<div class="col-lg-4">
													...
													<select name="call_types" class="form-control" required>
														<option value="Random Call" @if($audit_data->call_types == "Random Call") selected @endif>Random Call</option>
														<option value="Agent Error" @if($audit_data->call_types == "Agent Error") selected @endif>Agent Error</option>
													</select>
												...
												<div class="col-lg-4">
													...
													<input type="text" class="form-control" name="order_id"  value="{{$audit_data->order_id}}" required>
												</div>
												...
												<div class="col-lg-4">
													...
													<input type="text" class="form-control" name="vehicle_type" value="{{$audit_data->vehicle_type}}" name="call_time">
												</div>
												...
												<div class="col-lg-4">
													...
													<select class="form-control" id="caller_type_select" name="caller_type">
														<option value="Partner" @if($audit_data->caller_type == 'Partner') selected @endif>Partner</option>
														<option value="Customer" @if($audit_data->caller_type == 'Customer') selected @endif>Customer</option>
													</select>
													{{-- {{Form::select('caller_type',['Partner'=>'Partner','Customer'=>'Customer'],$audit_data->caller_type,[
														'placeholder' => 'Select Caller Type',
														'class' => 'form-control',
														'id' => 'caller_type_select',
														'required' => true,
													]) }} --}}
												</div>
												<div class="col-lg-4">
													...
													<select class="form-control" id="orderStage" name="order_stage">
														<option value="{{ $audit_data->order_stage }}">{{ $audit_data->order_stage }}</option>
													</select>
													@else
													<select class="form-control" id="orderStage" name="order_stage"></select>
													@endif
												</div>
												<div class="col-lg-4">
													<label>Issues:</label>
													...
													@else
													<select class="form-control" id="issue" name="issue"></select>
													@endif
												</div>
												<div class="col-lg-4">
													<label>Sub Issues:</label>
													@if ($audit_data->sub_issues != null)
													<select class="form-control" id="sub_issue" name="sub_issue">
														<option value="{{ $audit_data->sub_issues }}">{{ $audit_data->sub_issues }}</option>
													</select>
													@else
													<select class="form-control" id="sub_issue" name="sub_issue"></select>
													@endif
												</div>
												<div class="col-lg-4">
													<label>Scanerio:</label>
													@if ($audit_data->scanerio != null)
													<select class="form-control" id="scanerio" name="scanerio">
														<option value="{{ $audit_data->scanerio }}">{{ $audit_data->scanerio }}</option>
													</select>
													@else
													<select class="form-control" id="scanerio" name="scanerio"></select>
													@endif
												</div>
												<div class="col-lg-4">
													<label>Scanerio Codes:</label>
													@if ($audit_data->scanerio_codes != null)
													<select class="form-control" id="scanerio_code" name="scanerio_code">
														<option value="{{ $audit_data->scanerio_codes }}">{{ $audit_data->scanerio_codes }}</option>
													</select>
													@else
													<select class="form-control" id="scanerio_code" name="scanerio_code"></select>
													@endif
												</div>
												<div class="col-lg-4">
													<label>Error Reason Type:</label>
													<select class="form-control" id="error_reason_type" name="error_reason_type">
														@foreach($errorReasons as $val)
															<option value="{{$val}}" @if($val == $audit_data->error_reason_type ) selected @endif>{{$val}}</option>
														@endforeach
													</select>
												</div>
												<div class="col-lg-4">
													...
													<select class="form-control" id="error_reasons" name="error_reasons">
														<option value="{{ $audit_data->error_code_reasons }}">{{ $audit_data->error_code_reasons }}</option>
													</select>
													@else
													<select class="form-control" id="error_reasons" name="error_reasons"></select>
													@endif
													{{-- <input type="text" class="form-control" value="{{$audit_data->error_code_reasons}}"> --}}
													
												</div>
												<div class="col-lg-4">
													<label>Error Code:</label>
													@if ($audit_data->new_error_code != null)
													<select class="form-control" id="new_error_code" name="new_error_code">
														<option value="{{ $audit_data->new_error_code }}">{{ $audit_data->new_error_code }}</option>
													</select>
													@else
													<select class="form-control" id="new_error_code" name="new_error_code"></select>
													@endif
													{{-- <input type="text" class="form-control" value="{{$audit_data->new_error_code}}"> --}}
												</div>
												<div class="col-lg-4">
									...
									<div class="kt-portlet kt-portlet--mobile">
										...
										<div class="kt-portlet__body">
											...
											<div class="row" style="border-bottom:1px solid #ccc; padding: 20px 0; height: 100%; ">
												...
												<div class="col-md-10 sp-row">
													...
													<div class="row flex-container">
														...
														<div class="col-md-2">
															<p style="width: 100%; display: block; overflow-wrap: break-word;">{{($svalue['remark'])?$svalue['remark']:'-'}} </p>
														</div>
														<div class="col-md-1">
															<temp-audit-parameter-updater audit-id="{{$audit_data->id}}"
											...
											@endforeach
												<br/>
												{{Form::open([ 'method'  => 'post', 'url' => 'transfer_audit_from_temp_to_main_pool','onsubmit'=>"move_confirm()"])}}
											<input type="hidden" name="audit_id" value="{{Crypt::encrypt($audit_data->id)}}">
